...
class METRModel(FLModel):
    
    def __init__(self, device, scaler):  
        super(METRModel, self).__init__(device)
        self.loss_fn = self.masked_mae
        self.input_size=2
        self.hidden_size=64
        self.seq_len=12
        self.num_layers=1
        ...
        self.rnn = torch.nn.RNN(
            ...
            num_layers= self.num_layers,  # RNN隐藏层数
            batch_first=True
        ).to(device)  
        self.linear =  torch.nn.Linear(self.hidden_size, 1).to(device)

        self.bn0 = torch.nn.BatchNorm1d(self.hidden_size).to(device)
    ...
    def forward(self, x,h=None):
        # RNN layer
        out, h = self.rnn(x,h) # [b,12,64]

        
        # Batch Normalization
        out = out.transpose(1, 2).contiguous() # [b,64,12]
        out = self.bn0(out) # BN用于隐藏层
    ...
    def test(self, x, y, B):
        ...
        with torch.no_grad():
            for b in range(n_batches):                          
                output,_ = self.forward(x[b*B:(b+1)*B])               
                output[:,:,0] = self.scaler.inverse_transform(output[:,:,0])
                mae += self.masked_mae(output[:,:,0], y[b*B:(b+1)*B,:,0],0.0).item()
                mape += self.masked_mape(output[:,:,0], y[b*B:(b+1)*B,:,0],0.0).item()
                rmse += self.masked_rmse(output[:,:,0], y[b*B:(b+1)*B,:,0],0.0).item()
       
        self.train()
        
        return mae/n_batches, mape/n_batches,rmse/n_batches
    ...
    def masked_mse(self, preds, labels, null_val=np.nan):
    ...
    def masked_rmse(self, preds, labels, null_val=np.nan):
        return torch.sqrt(self.masked_mse(preds=preds, labels=labels, null_val=null_val))
    ...
    def masked_mape(self, preds, labels, null_val=np.nan):
        if np.isnan(null_val):
            mask = ~torch.isnan(labels)
        else:
            mask = (labels!=null_val)
...
class PEMSModel(FLModel):
    
    def __init__(self, device, scaler):  
        super(PEMSModel, self).__init__(device)
        self.loss_fn = self.masked_mae
        self.input_size=2
        self.hidden_size=32
        self.seq_len=12
    ...
    def test(self, x, y, B):
        ...
        with torch.no_grad():
            for b in range(n_batches-1):           
                output, h = self.forward(x[b*B:(b+1)*B],h)               
                output[:,:,0] = self.scaler.inverse_transform(output[:,:,0])
                mae += self.masked_mae(output[:,:,0], y[b*B:(b+1)*B,:,0],0.0).item()
                mape += self.masked_mape(output[:,:,0], y[b*B:(b+1)*B,:,0],0.0).item()
                rmse += self.masked_rmse(output[:,:,0], y[b*B:(b+1)*B,:,0],0.0).item()
        ...
        return mae/(n_batches-1), mape/(n_batches-1),rmse/(n_batches-1)

    def empty_step(self):
        x = torch.zeros((1,12,2), device=self.device, dtype=torch.float32)
        y = torch.zeros((1,12,1), device=self.device,dtype=torch.float32)
        output,_ = self.forward(x)
        loss = self.loss_fn(output[:,:,0], y[:,:,0])
        self.optim.zero_grad()
        loss.backward()        
        self.optim.step()
    ...
    def masked_rmse(self,preds, labels, null_val=np.nan):
        return torch.sqrt(self.masked_mse(preds, labels))
...
class NumpyModel():
    ...
    def notzero(self):
        ...
        for item in self.params:
            flag=flag or item.any()
        return flag
        
        # return np.array(self.params).any()
